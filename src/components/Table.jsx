...
const TableCardItem = ({
  ...
  return (
    <tr className=" border-b">
      ...
      <td className="text-text py-3.5 px-3 space-x-1.5 text-sm leading-5 font-normal flex items-center">
        <aside
          className={`w-2.5 h-2.5 ${
            successful ? "bg-green-600" : "bg-gray-300"
          } rounded-full`}
        ></aside>
        <span className="">{successful ? "Successful" : "Processing"}</span>
      </td>
      <td className="text-text py-3.5 px-3 text-sm leading-5 font-normal">
        {transactionID}
      </td>
      <td className="text-text py-3.5 px-3 text-sm leading-5 font-normal">
        {refundDate}
      </td>
      <td className="text-text py-3.5 px-3 text-sm leading-5 font-normal text-right">
        ₹{orderAmount}
      </td>
    </tr>
  );
    ...
    >
      ...
      <table className=" w-full text-left border-collapse ">
        <tr className="table-headers bg-[#F2F2F2] rounded">
          <th
            className="text-text py-1.5 px-3 border-[#F2F2F2] text-sm font-normal"
            style={{ width: "20%" }}
          >
            Order ID
          </th>
          ...
          <th
            className="text-text py-1.5 px-3 border-[#F2F2F2] text-sm font-normal text-right"
            style={{ width: "20%" }}
          >
            Order amount
          </th>
        ...
        {tableData.map(
          ({ orderId, successful, transactionID, refundDate, orderAmount }) => (
            <TableCardItem
              key={orderId}
              orderId={orderId}
